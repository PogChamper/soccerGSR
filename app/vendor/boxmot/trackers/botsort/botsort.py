...
from app.vendor.boxmot.trackers.botsort.botsort_utils import (joint_stracks,
                                                   remove_duplicate_stracks,
                                                   sub_stracks)
from app.vendor.boxmot.utils.matching import (embedding_distance, fuse_score,
...
class BotSort(BaseTracker):
    ...
    def __init__(
        ...
        **kwargs  # BaseTracker parameters
        ...
        # ReID module — vendored build does not bundle a backend.
        # Pass pre-computed embeddings via update(..., embs=...).
        self.proximity_thresh = proximity_thresh
        self.appearance_thresh = appearance_thresh
        self.with_reid = with_reid
        self.model = None
        if self.with_reid:
            # We keep the flag so that downstream code can supply external
            # embeddings; we just don't build any torch backbone here.
            pass
    ...
    def update(
        ...
        if self.with_reid and embs is None:
            if self.model is None:
                raise RuntimeError(
                    "BotSort: with_reid=True but no embeddings were provided. "
                    "Pass pre-computed embeddings via update(..., embs=...) or "
                    "set with_reid=False."
                )
